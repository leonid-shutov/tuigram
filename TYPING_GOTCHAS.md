...
is also declared as a global (in `types/global.d.ts`). TypeScript merged the two, and the
...
another file's local or a real ambient global from `types/*.d.ts`. It won't always announce
